...
-- Acompanhamentos (comentários nos chamados)
...
CREATE TABLE IF NOT EXISTS chamado_tecnicos (
  ...
  FOREIGN KEY (chamado_id) REFERENCES chamados(id) ON DELETE CASCADE ON UPDATE CASCADE,
  FOREIGN KEY (tecnico_id) REFERENCES usuarios(id) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Relacionamento muitos-para-muitos entre chamados e acompanhamentos
CREATE TABLE IF NOT EXISTS chamado_acompanhamentos (
  chamado_id        CHAR(36) NOT NULL,
  acompanhamento_id CHAR(36) NOT NULL,
  PRIMARY KEY(chamado_id, acompanhamento_id),
  FOREIGN KEY (chamado_id) REFERENCES chamados(id) ON DELETE CASCADE ON UPDATE CASCADE,
